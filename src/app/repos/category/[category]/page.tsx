...
const KIND = "repos" as const;

/** Resolve a URL slug back to the human category name. */
function categoryFor(slug: string) {
  return Object.keys(meta.categories.repos).find((name) => categorySlug(name) === slug) ?? null;
}

export function generateStaticParams() {
  return Object.keys(meta.categories.repos).map((name) => ({ category: categorySlug(name) }));
}
...
export async function generateMetadata({
  ...
  if (!name) return {};
  const count = meta.categories.repos[name];
